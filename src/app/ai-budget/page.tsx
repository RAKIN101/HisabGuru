...
import { Sparkles, TrendingUp, Calendar, Zap, Brain, Target, ArrowLeftRight, Check, Plus, MessageSquare, Edit3 } from "lucide-react";
...
export default function AIBudget() {
  ...
  const [directBudgetAmount, setDirectBudgetAmount] = useState("");
  ...
  const generateBudget = async (customInstructions?: string) => {
    ...
    try {
      ...
      // If direct budget amount is provided, use it
      let totalBudgetAmount = 7000; // Default
      if (directBudgetAmount && !isNaN(parseFloat(directBudgetAmount))) {
        totalBudgetAmount = parseFloat(directBudgetAmount);
        taskDescription += `. USER SPECIFIED BUDGET: ৳${totalBudgetAmount}`;
      }
...
6. Be flexible with budget limits to accommodate user requests like "budget should accommodate 1000tk more for my mother's birthday"
7. If user specifies a total budget amount, work within that constraint but make intelligent adjustments
  ...
  "considerations": ["Special events this month", "Other factors"]
      ...
      if (isInvalidBudgetData) {
        ...
        budgetData = {
          totalBudget: totalBudgetAmount,
          categories: [
            { category: "Food & Dining", suggested: Math.round(totalBudgetAmount * 0.3), reasoning: "Based on typical spending patterns" },
            { category: "Transport", suggested: Math.round(totalBudgetAmount * 0.1), reasoning: "Daily commute costs" },
            { category: "Education", suggested: Math.round(totalBudgetAmount * 0.2), reasoning: "Books and course materials" },
            { category: "Entertainment", suggested: Math.round(totalBudgetAmount * 0.1), reasoning: "Movies and social activities" },
            { category: "Shopping", suggested: Math.round(totalBudgetAmount * 0.15), reasoning: "Clothing and personal items" },
            { category: "Bills & Utilities", suggested: Math.round(totalBudgetAmount * 0.07), reasoning: "Mobile and internet expenses" }
          ],
          savingsGoal: Math.round(totalBudgetAmount * 0.08),
          tips: [
        ...
        if (budgetData.totalBudget === undefined) {
          ...
          budgetData.totalBudget = totalBudgetAmount; // Set the specified or default amount
        }
        ...
        if (!budgetData.savingsGoal) {
          budgetData.savingsGoal = Math.round(totalBudgetAmount * 0.08);
        }
...
If you continue to have issues, please contact support with the console logs.`);
  ...
  const handleDirectBudgetSubmit = () => {
    if (!directBudgetAmount || isNaN(parseFloat(directBudgetAmount))) {
      alert("Please enter a valid budget amount");
      return;
    }
    
    // Generate budget with the specified amount
    generateBudget(`User specified budget amount: ৳${directBudgetAmount}`);
  };
      ...
      >
            ...
            >
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-3 border-2 border-purple-200">
                ...
                <div className="flex gap-2">
                  <input
                    type="number"
                    value={directBudgetAmount}
                    onChange={(e) => setDirectBudgetAmount(e.target.value)}
                    ...
                    placeholder="Enter total budget amount (e.g., 8000)"
                  />
                  <button
                    onClick={handleDirectBudgetSubmit}
                    disabled={!directBudgetAmount || isNaN(parseFloat(directBudgetAmount))}
                ...
                <p className="text-[10px] text-gray-600 mt-2">
                  Enter a specific budget amount and the AI will create a plan within that constraint.
                </p>
              </div>
            </motion.div>
          )}
        ...
        <div className="flex gap-2 mb-2">
          ...
          >
            ...
            {showDirectInput ? "Hide" : "Specify"} Budget
          </motion.button>
          <motion.button
